...
def to_triplets(sampled_hops, scale_terms):
    ...
    for i, j in itertools.combinations(np.arange(1, sampled_hops.shape[1]), 2):
        ...
        # tmp = np.array([[x,math.exp(i), math.exp(j)] for x in range(sampled_hops.shape[0])])
        tmp = np.array([[x, i, j] for x in range(sampled_hops.shape[0])])
        tmp[tmp[:, 2] == (sampled_hops.shape[1] - 1), 2] = sampled_hops.shape[1] + 3
        # tmp[tmp[:,2] == (sampled_hops.shape[1]-1),2] = sampled_hops.shape[1]
    ...
    return np.row_stack(triplets), np.concatenate(triplet_scale_terms),np.row_stack(neigh_index).astype(np.float32)
...
def process_com_dblp():
    f = open('../data/com-dblp.ungraph.txt','r')
    lines = f.readlines()
    A=sp.lil_matrix((317080,317080)).astype(np.int8)
    mp = {}
    id = 0
    for i in lines:
        s = i.strip().split('\t')
    ...
    f = open('../data/com-dblp.top5000.cmty.txt', 'r')
    lines = f.readlines()
    labs = sp.lil_matrix((317080, 5000)).astype(np.int8)
    cnt = 0
    for i in lines:
        s = i.strip().split('\t')
        for j in s:
            labs[mp[j],cnt] = 1
        cnt += 1
    Z = sp.csr_matrix(labs)
    sp.save_npz('./com-dblp-A.npz', A)
    sp.save_npz('./com-dblp-Z.npz', Z)
...
def calcu_sim(query,dataset):
    return  query.dot(dataset.transpose())/np.sqrt(np.sum(dataset**2,1))


def get_relevant(query,dataset,qu_id,A):
    sim = []

    for j in range(dataset.shape[0]):
         sim.append(np.sum(np.square(query - dataset[j,:])))
         # sim.append( np.sum(query * dataset[j,:])
         #             /np.sqrt(np.sum(query**2))/np.sqrt(np.sum(dataset[j,:]**2)))
    # sim = query.dot(dataset)/M
    sim = np.array(sim)
    ...
    if np.sum(gt) ==0 :
        ...
        return  gt,0 #roc_auc_score(gt,-sim[index])

def get_quantization_relevant(dataset,pre_dis,qu_id,A,K):
    s = []
    p = np.arange(0,dataset.shape[1]) * K
   
    for i in range(dataset.shape[0]):
      s.append(np.sum(pre_dis[p+dataset[i,:]]))
    index = np.argsort(s)
    gt = []
...
def get_recommend_edge(B,op=0.5):
    B = sp.lil_matrix(B).astype(np.int8)
    cx = B.tocoo()
    recommend_edge = []
    last = -1
    tmp = []
    for i, j, v in itertools.izip(cx.row, cx.col, cx.data):
        if last != i and last >= 0:
            ...
            if n >= 1:
                ...
                np.random.shuffle(k)
                keep = k[:n]
                ok = np.array(tmp)[keep]
                for k in ok:
                    B[last, k] = 0
                    recommend_edge.append(k)
            tmp = []
            tmp.append( j)
            last = i
        else:
            tmp.append( j)
            last = i
    print( len(recommend_edge))
    return sp.csr_matrix(B),np.array(recommend_edge)

...
class Graph2Gauss:
    ...
    def __init__(self, input,A, X, L,z, K=3, data_ = '',p_val=0.10, p_test=0.05, p_nodes=0.0, n_hidden=None,
        ...
        # A,recom_edges_id = get_recommend_edge(A,0.1)
        ...
        if p_val > 0:
            ...
            # print self.val_ground_truth[:100]
            # raw_input("$$$$")
    ...
    def train(self, z, gpu_list='0'):
        ...
        train_op = tf.train.AdamOptimizer(learning_rate=1e-3).minimize(self.loss +
                         0.0*self.lamb_weight*self.loss_quatization+
                                (2-self.lamb_weight)*0*self.class_margin)
        #
        # train_op = tf.train.AdamOptimizer(learning_rate=1e-3).minimize(self.loss  *2  +
        #                                                                0.3 * self.loss_quatization +
        #                                                                0* self.class_margin)
        ...
        for epoch in range(self.max_iter):
            ...
            if epoch % 500 == -1:
                val_left = []
                val_right = []
                # aa = [ __ for __ in range(self.right_val.shape[0])]
                #
                # np.random.shuffle(aa)
                # aa = aa[:10000]
                # r_sample = self.right_val[aa,:]
                # l_sample = self.left_val[aa,:]
                qus2 = []
                qus1 = []
                ...
                # score = 1 - score #/ np.sqrt(np.sum(score ** 2))
                # score2 = score2 # / np.sqrt(np.sum(score2 ** 2))  # 1 - score2 / np.sqrt(np.sum(score2 ** 2))
                # print self.val_ground_truth.shape,self.val_ground_truth[:842].sum()

...
if input_X =='attribute':
    g2g = SNEQ(input=input_X,A=A, z= zz,data_ = name,X=X,L=128,batch = int(sys.argv[6]),
                      p_semi = p_lab, K=int(sys.argv[4]), p_val=0.05, p_test=0.1, p_nodes=0.0)
if input_X =='adjacent':
    g2g = SNEQ(input=input_X,A=A, z= zz, data_=name,batch = int(sys.argv[6]),
